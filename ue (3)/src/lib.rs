...
declare_id!("46Z2Aj7pmBmiMwwHW8M3ND7QQVBW6XgXeSMAR8zs86sU");
...
pub mod tournament_betting_system {
    ...
    pub fn create_tournament(
        ...
        tournament.price_ratio = price_ratio;

        // 验证 price_ratio
        require!(
            price_ratio >= 1 && price_ratio <= 10,
            TournamentError::InvalidPriceRatio
        );
        // 如果赛事方质押了matchp
        if stake_amount > 0 {
            // 转移matchp到合约账户
            ...
            msg!("Finished invoking create_ata CPI before");
            let cpi_ctx =
                CpiContext::new(ctx.accounts.token_program.to_account_info(), mint_accounts);
            token::mint_to(cpi_ctx, stake_amount * price_ratio)?;
            msg!("Finished invoking create_ata CPI");
            // 更新赛事为已质押状态
    ...
    /// vote
    /// 用户对特定轮次的团队进行vote，根据赛事是否质押决定使用wanzi或vote代币
    pub fn place_bet(ctx: Context<PlaceBet>, amount: u64) -> Result<()> {
        ...
        // 检查团队是否已被淘汰
        require!(
            !ctx.accounts.team.is_eliminated,
            TournamentError::TeamEliminated
        );
    ...
    /// 完成轮次
    /// 标记获胜团队，更新轮次状态
    pub fn complete_round(ctx: Context<CompleteRound>, winner_team_pubkey: Pubkey) -> Result<()> {
        let round = &mut ctx.accounts.round;
        // 检查轮次是否激活
        require!(round.is_active, TournamentError::RoundNotActive);
    ...
    pub fn close_tournament(ctx: Context<CloseTournament>) -> Result<()> {
        ...
        if is_staked {
            ...
            let seeds = &[b"tournament".as_ref(), authority_seeds, &[bump]];
            let signer = &[&seeds[..]];
...
pub struct CreateTournament<'info> {
    ...
    pub vote_mint_authority: Signer<'info>,
...
pub struct Tournament {
    ...
    /// PDA bump
    pub bump: u8,
    pub price_ratio: u64, // vote mint 比例
}
